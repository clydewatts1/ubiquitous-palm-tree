...
# This content should be in test_connection.py - will be moved there
test_connection_content = """\"\"\"Tests for Teradata connection management.\"\"\"
...
class TestTeradataConnection:
	...
	def test_init_with_missing_config(self, tmp_path):
		...
		with pytest.raises(TeradataConnectionError, match="Configuration file not found"):
	...
	def test_build_connection_string_missing_params(self, tmp_path):
		...
		with pytest.raises(TeradataConnectionError, match="Missing required parameters"):
...
# Write test_connection_content to file
with open('tests/test_connection.py', 'w', encoding='utf-8') as f:
